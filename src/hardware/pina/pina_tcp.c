...
static int pina_tcp_open(struct dev_context *devc)
{
...
static int pina_tcp_send_cmd(struct dev_context *devc,
				    const char *format, ...)
{
	int len, out;
...
static int pina_tcp_read_data(struct dev_context *devc, char *buf,
				     int maxlen)
{
	int len;
...
SR_PRIV int pina_tcp_drain(struct dev_context *devc)
{
	...
	do {
		...
		if (ret > 0)
			len += pina_tcp_read_data(devc, buf, 1024);
	} while (ret > 0);
...
static int pina_tcp_get_string(struct dev_context *devc, const char *cmd,
				      char **tcp_resp)
{
	...
	*tcp_resp = NULL;
	if (cmd) {
		if (pina_tcp_send_cmd(devc, cmd) != SR_OK)
			return SR_ERR;
	}

	timeout = g_get_monotonic_time() + devc->read_timeout;
	len = pina_tcp_read_data(devc, response->str,
...
static int pina_tcp_get_int(struct dev_context *devc,
				   const char *cmd, int *response)
{
	int ret;
	char *resp = NULL;

	ret = pina_tcp_get_string(devc, cmd, &resp);
	if (!resp && ret != SR_OK)
		return ret;
...
SR_PRIV int pina_tcp_detect(struct dev_context *devc)
{
	...
	ret = pina_tcp_get_string(devc, "version", &resp);
	if (ret == SR_OK && !g_ascii_strncasecmp(resp, "PINA", 4))
		ret = SR_OK;
	else
		ret = SR_ERR;
...
static int pina_start(struct dev_context *devc)
{
        ...
        return pina_tcp_send_cmd(devc, "get");
}
